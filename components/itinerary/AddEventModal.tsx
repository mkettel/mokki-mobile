...
export function AddEventModal({
  ...
  const [startTime, setStartTime] = useState<Date>(() => {
    const date = new Date();
    date.setHours(defaultHour || 9, 0, 0, 0);
    return date;
  });
  const [endTime, setEndTime] = useState<Date>(() => {
    const date = new Date();
    date.setHours((defaultHour || 9) + 1, 0, 0, 0);
    return date;
  });
  ...
  useEffect(() => {
    if (visible) {
      ...
      defaultStartTime.setHours(defaultHour || 9, 0, 0, 0);
      setStartTime(defaultStartTime);
      const defaultEndTime = new Date();
      defaultEndTime.setHours((defaultHour || 9) + 1, 0, 0, 0);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSubmit({
        ...
        endTime: hasEndTime ? formatTimeString(endTime) : undefined,
